...
export class EditNICModal extends React.Component<EditNICModalProps, EditNICModalState> {
    ...
    async save() {
        ...
        const configure_port_forwards =
            this.state.networkType == "user" && (!network.backend || network.backend == "passt");
        ...
        try {
            ...
            if (configure_port_forwards && mac) {
                ...
                await domainModifyXML(vm, dom => {
                    const elements =
                        dom.querySelectorAll(`devices interface:has(mac[address="${CSS.escape(mac)}" i]) portForward`);
