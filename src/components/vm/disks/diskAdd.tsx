...
function update_AdditionalOptions(value: DialogValue<AdditionalOptionsValue>, vm: VM, device: VMDiskDevice) {
    const { _bus_types } = value.get();
    const bus_type = getBusType(vm, device);

    if (!_bus_types.includes(bus_type))
        _bus_types.push(bus_type);

    value.sub("_bus_types").set(_bus_types);
    value.sub("bus_type").set(bus_type);
}

function validate_AdditionalOptions(value: DialogValue<AdditionalOptionsValue>) {
    value.sub("serial").validate(v => {
        if (v != clearSerial(v))
...
const AdditionalOptions = ({
    ...
    value: DialogValue<AdditionalOptionsValue>,
}) => {
    const { expanded, bus_type, serial, _bus_types } = value.get();
    ...
    return (
        <ExpandableSection
            ...
            onToggle={() => value.sub("expanded").set(!expanded)}
            isExpanded={expanded}
            ...
            <Form onSubmit={e => e.preventDefault()} isHorizontal>
                <Grid hasGutter md={6}>
                    <DialogDropdownSelectObject
                        label={_("Cache")}
                        value={value.sub("cache_mode")}
                        options={diskCacheModes}
                    />
                    <DialogDropdownSelectObject
                        label={_("Bus")}
                        value={value.sub("bus_type")}
                        options={_bus_types}
                    />
                </Grid>
                <DialogTextInput
                    label={_("Disk identifier")}
                    value={value.sub("serial")}
...
function validate_CreateNew(value: DialogValue<CreateNewValue | string>) {
    const val = value.get();
    if (typeof val == "string")
        return;

    const vc_value = value.at(val);
    validate_VolumeCreate(vc_value.sub("volume"));
}

const CreateNew = ({
    value,
} : {
    value: DialogValue<CreateNewValue | string>,
}) => {
    const val = value.get();
    if (typeof val == "string")
        return null;
    ...
    function update_pool(p: StoragePool) {
        update_VolumeCreate(vc_value.sub("volume"), p);
    }
    ...
    return (
        <>
            <DialogDropdownSelectObject
                label={_("Pool")}
                value={vc_value.sub("pool", update_pool)}
                options={_pools}
                option_label={p => p.name}
            />
            <VolumeCreate value={vc_value.sub("volume")} />
        </>
    );
};
...
const UseExisting = ({
    ...
    value: DialogValue<UseExistingValue | string>,
}) => {
    const val = value.get();
    if (typeof val == "string")
        return null;

    const ue_value = value.at(val);
    const { pool, volume, _pools, _vms } = val;

    function update_pool(p: ExistingPool) {
        ue_value.sub("volume").set(p.volumes[0]);
    }
    ...
    return (
        <>
            <DialogDropdownSelectObject
                label={_("Pool")}
                value={ue_value.sub("pool", update_pool)}
                options={_pools}
                option_label={p => p.name}
            />
            <DialogDropdownSelectObject
                label={_("Volume")}
                value={ue_value.sub("volume")}
...
function validate_CustomPath(value: DialogValue<CustomPathValue>) {
    const { _fileInfoCache } = value.get();
    value.sub("file").validate_async(0, async file => {
        if (!file)
...
const CustomPath = ({
    ...
    value: DialogValue<CustomPathValue>,
    hideDeviceRow?: boolean,
}) => {
    function update_file(val: string) {
        if (val.endsWith(".iso"))
            value.sub("device").set("cdrom");
    }
    ...
    return (
        <>
            <FormGroup
                label={_("Custom path")}
                id={value.sub("file").id()}
            >
                <FileAutoComplete
                    placeholder={_("Path to file on host's file system")}
                    onChange={(val: string) => value.sub("file", update_file).set(val)}
                    superuser="try"
                />
                <DialogHelperText value={value.sub("file")} />
            </FormGroup>
            {
                !hideDeviceRow &&
                    <DialogDropdownSelect
                        label={_("Device")}
                        value={value.sub("device")}
                        options={
                            [
...
export const AddDisk = ({
    ...
    function validate(dlg: DialogState<AddDiskValues>) {
        if (dlg.values.mode == CREATE_NEW)
            validate_CreateNew(dlg.value("create_new"));
        if (dlg.values.mode == CUSTOM_PATH)
            validate_CustomPath(dlg.value("custom_path"));
        if (!isMediaInsertion)
            validate_AdditionalOptions(dlg.value("additional_options"));
    }
    ...
    function update_bus() {
        ...
        if (device)
            update_AdditionalOptions(dlg.value("additional_options"), vm, device);
    }
            ...
            <>
                <Form onSubmit={e => e.preventDefault()} isHorizontal>
                    <DialogRadioSelect
                        isInline
                        label={_("Source")}
                        value={dlg.value("mode", update_bus)}
                        options={mode_options}
                    />
                    {
                        mode === CREATE_NEW &&
                            <CreateNew value={dlg.value("create_new", update_bus)} />
                    }
                    {
                        mode === USE_EXISTING &&
                            <UseExisting value={dlg.value("use_existing", update_bus)} />
                    }
                    {
                        mode === CUSTOM_PATH &&
                            <CustomPath value={dlg.value("custom_path", update_bus)} hideDeviceRow={isMediaInsertion} />
                    }
                    {
                ...
                { !isMediaInsertion &&
                    <AdditionalOptions value={dlg.value("additional_options")} />
                }
            </>
        );
    }
        ...
        try {
            ...
            if (values.mode === CREATE_NEW && typeof values.create_new != "string") {
                ...
                await domainAttachDisk({
                    ...
                    // XXX - The cache is guaranteed to have a result
                    // because the validation function is guaranteed
                    // to have been run for "params.file". This could
                    // be made less magic.
                    format: params._fileInfoCache[params.file].format,
                    shareable: false,
                    ...common
                });
            } else if (values.mode == USE_EXISTING && typeof values.use_existing != "string") {
