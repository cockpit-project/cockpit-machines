...
import { MaybePendingDescription } from '../../common/needsShutdown.jsx';
...
export const VmDisksCard = ({
    ...
    const rows: ListingTableRowProps[] = disks.map(disk => {
        const inactive_disk = disk.target in vm.inactiveXML.disks ? vm.inactiveXML.disks[disk.target] : null;
        ...
        const expandedContent = (
            <DescriptionList isAutoFit>
                ...
                <MaybePendingDescription
                    active={disk}
                    inactive={inactive_disk}
                    idPrefix={`${idPrefixRow}-access`}
                    term={_("Access")}
                    accessor={disk => disk.readonly ? "read-only" : disk.shareable ? "shareable" : "writeable"}
                    formatter={mode => mode == "read-only" ? _("Read-only") : mode == "shareable" ? _("Concurrently writeable") : _("Writeable")}
                />
