...
export const VmDisksCard = ({
    ...
    const rows: ListingTableRowProps[] = disks.map(disk => {
        ...
        columns.push({ title: <DiskSourceAbbrev diskSource={disk.source} /> });
        columns.push({
            title: <StorageUnit
                       value={disk.capacity}
        ...
        const expandedContent = (
            <DescriptionList isHorizontal>
                { !isNaN(Number(disk.used)) &&
                    <Description term={_("Used on host")}>
                        <StorageUnit value={disk.used} id={`${idPrefixRow}-used`} key={`${idPrefixRow}-used`} />
                    </Description>
                }
