...
export class EditDiskModal extends React.Component<EditDiskModalProps, EditDiskModalState> {
    ...
    render() {
        ...
        const disk2 = vm.disks[disk.target]; // XXX - do we always have disk === disk2?

        const defaultBody = (
            <Form isHorizontal>
                <NameRow idPrefix={idPrefix}
                         diskType={disk2.type || ""}
                         name={getDiskFullName(disk2)} />
        ...
        return (
            <Modal position="top" variant="medium" id={`${idPrefix}-dialog`}
                   ...
                   title={cockpit.format(_("Edit $0 attributes"), getDiskPrettyName(disk2))}
                   footer={
                       <>
