...
const AddHostDev = ({ idPrefix, vm, nodeDevices, close }) => {
    ...
    useEffect(() => {
        setSelectableDevices(getSelectableDevices(devicesHaveAChild(nodeDevices), vm, type));
    }, []);

    const setTypeWrapper = (newType) => {
        setSelectableDevices(getSelectableDevices(devicesHaveAChild(nodeDevices), vm, newType));
        setType(newType);
    };
