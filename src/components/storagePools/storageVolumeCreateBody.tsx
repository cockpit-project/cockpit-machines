...
import {
    DialogValue,
    DialogTextInput, DialogFormSelect,
    DialogHelperText,
} from '../common/dialog';
...
export function update_VolumeCreate(value: DialogValue<VolumeCreateValue>, storagePool: StoragePool) {
    // XXX - keep format if still valid
    value.sub("format").set(getDefaultVolumeFormat(storagePool) || "");
    value.sub("_storagePool").set(storagePool);
}

export function validate_VolumeCreate(value: DialogValue<VolumeCreateValue>) {
...
export const VolumeCreate = ({
    ...
    value: DialogValue<VolumeCreateValue>
}) => {
    const { size, unit, format, _storagePool } = value.get();
    ...
    if (validVolumeFormats) {
        if (!format)
            ...
            formatRow = (
                <DialogFormSelect
                    label={_("Format")}
                    value={value.sub("format")}
                >
                    {
                        validVolumeFormats.map(format =>
                            <FormSelectOption value={format} key={format} label={format} />)
                    }
                </DialogFormSelect>
            );
    }
    ...
    return (
        <>
            ...
            />
            { /* XXX - use a DialogSizeInput or DialogSizeSlider */ }
            <Grid hasGutter md={6}>
                <FormGroup
                    id={value.id("size-group")}
                    label={_("Size")}>
                    <InputGroup>
                        <TextInput
                            id={value.sub("size").id()}
                            type="number"
                            inputMode='numeric'
                            pattern="[0-9]*"
                            ...
                            min={0}
                            max={volumeMaxSize}
                            onChange={(_, val) => value.sub("size").set(parseInt(val))}
                        />
                        <DialogFormSelect
                            className="ct-machines-select-unit"
                            value={value.sub("unit")}
                        >
                    ...
                    <DialogHelperText value={value.sub("size")} />
                </FormGroup>
                {formatRow}
            </Grid>
        </>
...
// Legacy implementation of the stuff above, will be removed once unused.
